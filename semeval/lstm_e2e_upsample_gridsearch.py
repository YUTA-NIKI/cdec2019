                            ...
                            for lr in lr_list:
                                for l2 in l2_list:
                                    for epoch in epoch_list:
                                        ...
                                        df = pd.read_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_e2e_upsample.csv'))
                                        ...
                                        df.append(write_ser, ignore_index=True).to_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_e2e_upsample.csv'), index=False)
                                        del df, net, criterion, optimizer
                                    
        del train_loader, test_loader
        i += 1
        notice = '{}/{} gridsearch of LSTM for semeval with upsampling is finished!!!'.format(i, len(batch_list))
