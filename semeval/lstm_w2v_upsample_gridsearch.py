                            ...
                            for lr in lr_list:
                                for l2 in l2_list:
                                    for epoch in epoch_list:
                                        ...
                                        df = pd.read_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_w2v_upsample.csv'))
                                        ...
                                        df.append(write_ser, ignore_index=True).to_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_w2v_upsample.csv'), index=False)
                                        del df, net, criterion, optimizer

            del train_loader, test_loader
        i += 1
        notice = '{}/{} gridsearch is of LSTM \w w2v for semeval upsampled data finished!!!'.format(i, len(w2v_list))
