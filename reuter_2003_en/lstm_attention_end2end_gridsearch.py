...
def main():
    ...
    optimize_list = ['sgd', 'adam']  # optimizer
    ...
    for batch_size in tqdm(batch_list):
            ...
            for hidden_dim in hidden_list:
                for dropout_rate in dropout_list:
                    for activate in activate_list:
                        for lr in lr_list:
                            for l2 in l2_list:
                                for optim_name in optimize_list:
                                    for epoch in epoch_list:

                                        df = pd.read_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_attention_end2end.csv'))
                                        if len(df.query('epoch == @epoch & batch_size == @batch_size & embedding_dim == @emb_dim & hidden_dim == @hidden_dim & activate_func == @activate & optimizer == @optim_name & learning_rate == @lr & l2_regular == @l2 & dropout_rate == @dropout_rate')) == 1:
                                            continue
                                        del df
                                        ...
                                        if optim_name == 'sgd':
                                            ...
                                            optimizer = optim.Adam(net.patameters(), lr=le, weight_decay=l2)
                                        ...
                                        df = pd.read_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_attention_end2end.csv'))
                                        write_ser = pd.Series([
                                            epoch, batch_size, emb_dim, hidden_dim, activate, optim_name, lr, l2, dropout_rate, result[0], result[1], result[2], result[3]
                                        ], index = df.columns)
        ...
        i += 1
        notice = '{}/{} gridsearch of LSTM \w self-attention for reuter 2003 en is finished!!!'.format(i, len(batch_list))
