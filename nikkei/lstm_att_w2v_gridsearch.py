...
# dict_path = '-d /usr/lib/x86_64-linux-gnu/mecab/dic/mecab-ipadic-neologd'    # path ofsparrow
...
def main():
    ...
    w2v_list =['w2v_jawiki_d200_w12.model', 'w2v_jawiki_d300_w12.model', 'w2v_jawiki_d400_w12.model']
    ...
    for w2v_name in tqdm(w2v_list):
        w2v = word2vec.Word2Vec.load(os.path.join("/home/b2018yniki/data/word2vec", w2v_name))
        
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=2019)
    
        X_train = torch.LongTensor(padding(df2indexseq(X_train, w2v))).to(device)
        y_train = torch.LongTensor(y_train.values).to(device)
        X_test  = torch.LongTensor(padding(df2indexseq(X_test, w2v))).to(device)
                            ...
                            for lr in lr_list:
                                for l2 in l2_list:
                                    for epoch in epoch_list:
                                        ...
                                        df = pd.read_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_att_w2v.csv'))
                                        ...
                                        df.append(write_ser, ignore_index=True).to_csv(os.path.join(os.getcwd(), 'results/gridsearch_lstm_att_w2v.csv'), index=False)
                                        del df, net, criterion, optimizer

            del train_loader, test_loader
        i += 1
        notice = '{}/{} gridsearch of LSTM \w self-attention \w w2v for nikkei data is finished!!!'.format(i, len(w2v_list))
